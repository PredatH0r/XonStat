...
def get_overall_stats(player_id):
    ...
    for row in raw_stats:
        ...
        os = OverallStats(total_kills=row.total_kills,
                ...
                last_played_epoch=timegm(row.last_played.timetuple()) if row.last_played is not None else None,
                last_played_fuzzy=pretty_date(row.last_played),
                total_playing_time=row.total_playing_time,
                total_playing_time_secs=int(datetime_seconds(row.total_playing_time)) if row.total_playing_time is not None else 0,
...
def get_elos(player_id):
    ...
    found_max_elo = 0
    for row in raw_elos:
        if found_max_elo == 0 or row.g2_r > found_max_elo:
            elos['overall'] = row
            found_max_elo = row.g2_r 
